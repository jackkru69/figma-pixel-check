...
// report's 0.5 % mark (0.05 pp counts as weak). A signal only in other sections is
...
function verdict(mutation, base, mutated) {
  ...
  const STRONG_TYPES = ['geometry', 'pixel', 'spacing', 'color'];
...
const lines = [
  ...
  `or a colour share over the report's ${COLOR_MARK} % mark. **Weak**: only a smaller signal there (mismatch ≥ ${WEAK} pp,`,
...
for (const kind of [...kinds, null]) {
  ...
  lines.push(
    `| ${kind ?? '**all**'} | ${list.length} | ${count(list, 'detected')} | ${by(list, 'geometry')} | ${by(list, 'spacing')} | ${by(list, 'pixel')} | ${by(list, 'color')} | ${count(list, 'weak')} | ${count(list, 'misattributed')} | ${count(list, 'missed')} |`,
  );
}
